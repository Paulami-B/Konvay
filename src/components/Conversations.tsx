...
import { Conversation } from "@/utils/store/chatStore";
import EmptyConversations from "./EmptyConversations";

export default function Conversations() {
  const [showModal, setShowModal] = useState(false);
  const { currentUser } = useAuthStore();
  const myConversations : Conversation[] | undefined = useQuery(api.conversations.getMyConversations, currentUser ? {uid: currentUser.uid} : "skip");
  
  return (
    <div className="h-screen w-full p-4 bg-orange-50/40 dark:bg-gray-800 flex flex-col">
      <div className="top-0 h-fit sticky bg-[#FFFCF8] dark:bg-gray-800 z-50">
        <div className="flex justify-between items-center text-2xl">
          <div className="text-4xl font-bold dark:text-orange-100 my-2">Chats</div>
          <BsPersonFillAdd className="text-3xl cursor-pointer dark:text-gray-100 hover:text-orange-500 dark:hover:text-marigold"
          onClick={() => setShowModal(true)} />
        </div>
        {showModal && (
          <Modal onClose = {() => setShowModal(false)}>
            <Contacts onSuccess = {() => setShowModal(false)} />
          </Modal>
        )}
        <div className="flex items-center gap-2 bg-orange-50 dark:bg-marigold rounded-lg p-3 w-full">
